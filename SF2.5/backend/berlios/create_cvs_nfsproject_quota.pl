...
# http://www.berlios.de/
#
# Create CVS project quota on NFS server
...
while ($ln = shift(@group_dump)) {
	...
	print("/usr/sbin/edquota -p ".$config{'cvs_quota_temp_id'}." $groupid (cvs$groupname)\n");
	system("/usr/sbin/edquota -p ".$config{'cvs_quota_temp_id'}." $groupid");
}
